...
class Model:
	...
	def get_model_name(self):
		if "resnext" in self.modeldir.lower():
			...
			raise Exception("Cannot load the current model")
...
class MainWindow(QtWidgets.QMainWindow):
	...
	def SelectModel(self):
		modelname = QtWidgets.QFileDialog.getOpenFileName()
		self.model = Model(modelname[0])
		## Load classes
	...
	def SelectImage(self):
		imagename = QtWidgets.QFileDialog.getOpenFileName()
		self.image = imagename[0]
		print("Image loaded")
		if self.model != None:
			self.predict.setStyleSheet("background-color: green; font: white")
